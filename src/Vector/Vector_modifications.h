...
typename Vector<T>::Iterator
	Vector<T>::insert(typename Vector<T>::Iterator itPos, const T *b, const T *e)
{
	typename Vector<T>::Rank r = itPos - begin();
	ASSERT_DEBUG(itPos <= end() && itPos >= begin(), "Invalid pos Iterator");
...
inline typename Vector<T>::Iterator Vector<T>::insert(
	typename Vector<T>::Iterator itPos,
	const typename Vector<T>::Iterator &b,
...
template <typename T>
typename Vector<T>::Iterator
	Vector<T>::erase(typename Vector<T>::Iterator itBegin, typename Vector<T>::Iterator itEnd)
{
	typename Vector<T>::Rank rB = itBegin - begin(), rE = itEnd - begin();
	ASSERT_DEBUG(
		((itBegin >= begin() && itBegin <= end()) && (itEnd > begin() && itEnd <= end()) &&
		 itBegin <= itEnd),
...
void Vector<T>::remove(const T &value)
{
	...
	{
		if (_elem[i] != value) { _elem[i - k] = _elem[i]; }
		else
			++k;
	}
