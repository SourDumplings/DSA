...
namespace DSA
{
    ...
    public:
        ...
        TreeSetIterator<T> &operator++()
        {
            ...
            if (!_it->next())
            {
                // 红黑树最右侧结点再前移即为尾后迭代器
                _isEnd = true;
                _it = BiIterator<RedBlackTreeNode<T>>(reinterpret_cast<RedBlackTreeNode<T> *>(_pRBT->root()));
            }
            else
            {
                _it = BiIterator<RedBlackTreeNode<T>>(reinterpret_cast<RedBlackTreeNode<T> *>(_it->next()));
            }
            return *this;
        }
        ...
        TreeSetIterator<T> &operator--()
        {
            ...
            {
                _it = BiIterator<RedBlackTreeNode<T>>(reinterpret_cast<RedBlackTreeNode<T> *>(_it->prev()));
            }
            return *this;
        }
        TreeSetIterator<T> operator--(int)
    ...
    private:
        ...
        {
            RedBlackTreeNode<T> *pLastNode = reinterpret_cast<RedBlackTreeNode<T> *>(_pRBT->root());
            while (pLastNode->right_child())
            {
                pLastNode = reinterpret_cast<RedBlackTreeNode<T> *>(pLastNode->right_child());
            }
            return pLastNode;
        }
    };
